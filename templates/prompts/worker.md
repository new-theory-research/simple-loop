...
1. **Read state.** Read these files:
   ...
   - `feedback.md` in the card directory (`wiki/briefs/cards/<brief-id>/feedback.md`) if it exists. **This file is load-bearing.** Any directive marked MUST-FIX takes priority over tasks_remaining — resolve it before picking up new work. Record closure of each directive in progress.json learnings (e.g. "feedback.md MUST-FIX 'X': fixed by doing Y"). Skipping a MUST-FIX and proceeding as if the previous cycle passed is the failure mode feedback.md exists to prevent.
...
5. **Commit.** Stage your changes and commit with a descriptive message. You are on a brief branch — commit there. Do NOT push; the daemon handles pushing.
   - **`.loop/state/progress.json` is gitignored** — a plain `git add`/`git add -A` silently skips it. But the daemon reads a brief's progress/status from the **committed branch** (`git show`), so your progress update must actually land in the commit. Force-add it every time you commit progress: `git add -f .loop/state/progress.json`. (This is safe: you are on a brief branch, never main; the daemon strips the file back off main at merge.)
