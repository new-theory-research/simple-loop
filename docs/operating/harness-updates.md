...
## ntfy notification policy (`NTFY_EVENTS`)

`notify()` in `lib/daemon.sh` takes a **class** as its first argument — `notify <class> <message>` — and only pushes when that class appears in the `NTFY_EVENTS` allowlist (`.loop/config.sh`, default `brief_started,brief_escalated,brief_completed,queue_stuck`). `NTFY_TOPIC` empty still means zero pushes, checked first, so the gate is free when notifications are off. The five classes: `brief_started` (a brief dispatched — one push per dispatch, not per worker iteration), `brief_escalated` (a brief was parked/escalated — repeat-failure, over-budget, sync/rebase/staleness, capability-gap, 3× worker failures), `brief_completed` (a brief finished — auto-merge queued or arrived at awaiting-review), `queue_stuck` (the daemon is alive but a queued brief has sat undispatched with nothing active for `QUEUE_STUCK_TICKS` ticks — one alarm, re-armed on the next successful dispatch), and `ops` (operational chatter: queen kill/fail, circuit-breaker trip/pause/resume, per-iteration and validator noise, merge confirmations). `ops` is deliberately **absent** from the default list, so day-to-day the phone only lights up on Mattie's four lifecycle moments. Add `ops` to `NTFY_EVENTS` to hear everything again. Note: daemon-*dead* detection is not an ntfy class — a dead daemon can't notify itself; that rides a separate apiary sentinel.
