...
6. **brief-151 (make a program lane a real lock — partition + atomic claim)** — keystone of the `remote-queens` lane. Turns the `Program:` card field (currently read by zero code) into an enforced partition: `enumerate_dispatchable()` gains a `lane` arg, the daemon honors `--lane <name>`, and a brief is claimed via a pushed git ref (`refs/claims/<brief>`, `--force-with-lease`) BEFORE any worktree exists — so two daemons on one repo+lane can never both execute one brief. With no `--lane`, single-daemon behavior is byte-for-byte unchanged. Self-modifying (edits `lib/queue.py`/`lib/daemon.sh`/`lib/actions.py`); new behavior goes live only after `install.sh` re-copy + service restart. Opus, parallel-safe, 3 cycles. Cycle ceiling 6. Auto-merge: false, Human-gate: review. Edit-surface: `lib/queue.py`, `lib/daemon.sh`, `lib/actions.py`, `lib/claim.py` (new), `lib/tests/test_lane_and_claim.py` (new), `install.sh`. Depends-on: _none_. Canonical at `wiki/briefs/cards/brief-151-lane-lock-and-claim/index.md`.
