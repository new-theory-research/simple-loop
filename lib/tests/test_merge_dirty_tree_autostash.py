...
class TestMergeExistingGatesPreserved(unittest.TestCase):
    ...
    def test_real_content_conflict_aborts_with_dirty_tree(self):
        """Real content conflict → merge --abort, returns False, even with dirty tree.

        Set up a conflict: same file edited differently on main and branch.
        The autostash restores dirty files before abort returns, then the
        conflict path fires normally.
        """
        tmp = Path(tempfile.mkdtemp())
        ...
        # Branch: edit conflict.txt one way
        _git(project, "checkout", "-b", BRIEF)
        ...
        try:
            ...
            # git must be in a clean state (no in-progress merge, no stash left dangling)
            # git status --short should not contain 'UU' (unmerged) markers
